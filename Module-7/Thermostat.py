...
class TemperatureMachine(StateMachine):
	...
	## update the lights on the thermostat
        self.updateLights()
	...
	## turn red LED off for exiting heat state
        redLight.off()
	...
	## turn blue LED off for exiting cool state
        blueLight.off()
	...
	## turn red and blue LED light off when entering off state
        redLight.off()
        blueLight.off()
	...
	## change the state of thermostat
        self.send("cycle")
	...
	## update (increase) the setPoint of the thermostat
	...
	## update (decrease) the setPoint of the thermostat
        self.setPoint = self.setPoint - 1
	## update the LED lights
        self.updateLights()
	...
	## identifier for heat state
        if(self.current_state.id == 'heat'):
            # thermostat is set to heat mode
            if(temp >= self.setPoint):
                # temperature is at or above the setPoint (heat off)
                redLight.on()
            else:
                # temperature is less than set point (heat on)
                redLight.pulse()
	## identifier for cool state
        elif(self.current_state.id == 'cool'):
            # thermostat is set to cool mode
            if(temp <= self.setPoint):
                # temperature is at or below the setPoint (cool off)
                blueLight.on()
            else:
                # temperature is above setPoint (cool on)
                blueLight.pulse()
        else:
            # thermostat is off
            pass
    ...
    ##
    ##  output string for the Thermostat Server
    ##
    def setupSerialOutput(self):
	## out current state of the thermostat, temperature in Fahrenheit, and current setPoint
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Setup display line 1 - showing current date and time
            lcd_line_1 = current_time.strftime('%b %d  %H:%M:%S\n')
    
            ## Setup Display Line 2 - showing current temperature in Fahrenheit
            if(altCounter < 6):
                lcd_line_2 = f"T:{self.getFahrenheit():0.1f}F"
                altCounter = altCounter + 1

	    ## Setup Alternate Display Line 2 - current thermostat state and setPoint 
            else:
            ...
            if((counter % 30) == 0):
		## send current state information through the serial port/UART
...
## change state of the thermostat when pressed
...
##
redButton = Button(25)
## increase the setPoint by one degree Fahrenheit when pressed
...
## decrease the setPoint by one degree Fahrenheit when pressed
